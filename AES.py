...
"""
States a state in as an in_array. The in_array should be a Rijndael state
...
def inv_cipher(in_array, key, nb = 4, nr = 10):
    ...
    for i in range(nr-1, 0, -1):
        ...
        state = add_round_key(state, [word_array[(i*nb)+j] for j in range(4)])
        state = inverse_mix_columns(state, 4)
    state = shift_rows(state, 4, -1)
    state = sub_bytes(state, 4, sbox_inv)
    state = add_round_key(state, [word_array[i] for i in range(4)])
    return state
...
def mix_columns(state, num_blocks):
    ...
    return state
def inverse_mix_columns(state, num_blocks):
    for r in range(num_blocks):
        state[r] = matrix_mult([state[r]], inverse_column_matrix, plusGF256, multGF256)[0]
    return state
def matrix_mult(B, A, plus, multi):
    if (len(B[0]) != len(A)):
        raise Exception ("Matrix Dimension Error")
        sys.exit(1)
    #build a matrix of len(B) * len(A[0])
...
def print_inverted_arr(arr):
    for i in arr:
        for j in i:
            print invert_byte(j)
#takes a bytestring as imput and yields bytes until empty. 
def sub_word(word):
...
def run_aes_input(string, key, aes_func, nb = 4):
    state_generator = pack_input(string, nb)
    ret = ''
    while True:
        try:
...
def run_aes(filename, key, aes_func, nb = 4):
    output_file = raw_input("What do you want to name the output file: ")
...
if __name__ == "__main__":
    ...
    t = raw_input("What string do you want to try and encrypt: ")
    print t
    z = run_aes_input(t, key, cipher)
    out = run_aes_input(z, key, inv_cipher)
    print t, z, out